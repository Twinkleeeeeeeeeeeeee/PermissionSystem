...
<SCRIPT type="text/javascript">
    ...
    $(function(){
        ...
        $("ul li span").click(function(){
            $("ul li ul").not($(this).next()).hide();
            $("ul li span").not($(this).next()).removeClass("current");
...
<BODY style="overflow-y: hidden;" class="easyui-layout" scroll="no" >
	<DIV
		...
		<SPAN style="padding-right: 20px; float: right;" class="head">
			...
			用户权限系统</SPAN> <SPAN style="padding-left: 15px;" id="News"></SPAN>
	</DIV>
					...
					<li>
						<a title="${menu.name }" ref="1_1" href="#"
						   rel="${baseurl }/${menu.url }" icon="icon-log"><span
								class="icon icon-log">&nbsp;</span><span class="nav">
							<a href=javascript:addTab('${menu.name }','${baseurl }/${menu.url }')>${menu.name }</a></span></a>
					</li>
				</c:forEach>
				</ul>
			</c:if>
